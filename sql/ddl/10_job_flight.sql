create procedure job_flight(load_type text DEFAULT 'full'::text)
    language plpgsql
as
$$
DECLARE
    v_proc_name       CONSTANT TEXT := 'JOB_FLIGHT';
    V_TABLE          CONSTANT TEXT := 'ft_flight';
    V_TABLE_1           CONSTANT TEXT := 'str_flight';
    V_TABLE_2          CONSTANT TEXT := 'tr_flight';
...
EXECUTE
    ...
    FROM (
        SELECT
            *,
            ROW_NUMBER() OVER (PARTITION BY flight_id ORDER BY created_at DESC) AS rn
    ...
    EXECUTE
        ...
        'WHERE tgt.flight_id = src.flight_id ';
...
alter procedure job_flight(text) owner to admin;
