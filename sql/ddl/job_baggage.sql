create procedure job_baggage(load_type text DEFAULT 'full'::text)
    language plpgsql
as
$$
DECLARE
    v_proc_name       CONSTANT TEXT := 'JOB_BAGGAGE';
    V_TABLE          CONSTANT TEXT := 'ft_baggage';
    V_TABLE_1           CONSTANT TEXT := 'str_baggage';
    V_TABLE_2          CONSTANT TEXT := 'tr_baggage';
...
EXECUTE
    ...
    FROM (
        SELECT
            *,
            ROW_NUMBER() OVER (PARTITION BY baggage_id ORDER BY created_at DESC) AS rn
    ...
    EXECUTE
        ...
        'WHERE tgt.baggage_id = src.baggage_id ';
...
alter procedure job_baggage(text) owner to admin;
