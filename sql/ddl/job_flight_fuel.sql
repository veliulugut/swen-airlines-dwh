create procedure job_flight_fuel(load_type text DEFAULT 'full'::text)
    language plpgsql
as
$$
DECLARE
    v_proc_name       CONSTANT TEXT := 'JOB_FLIGHT_FUEL';
    V_TABLE          CONSTANT TEXT := 'ft_flight_fuel';
    V_TABLE_1           CONSTANT TEXT := 'str_flight_fuel';
    V_TABLE_2          CONSTANT TEXT := 'tr_flight_fuel';
...
EXECUTE
    ...
    FROM (
        SELECT
            *,
            ROW_NUMBER() OVER (PARTITION BY fuel_id ORDER BY created_at DESC) AS rn
...
alter procedure job_flight_fuel(text) owner to admin;
