create procedure job_maintenance_event(load_type text DEFAULT 'full'::text)
    language plpgsql
as
$$
DECLARE
    v_proc_name       CONSTANT TEXT := 'JOB_MAINTENANCE_EVENT';
    V_TABLE          CONSTANT TEXT := 'ft_maintenance_event';
    V_TABLE_1           CONSTANT TEXT := 'str_maintenance_event';
    V_TABLE_2          CONSTANT TEXT := 'tr_maintenance_event';
...
EXECUTE
    ...
    FROM (
        SELECT
            *,
            ROW_NUMBER() OVER (PARTITION BY maintenance_id ORDER BY created_at DESC) AS rn
    ...
    EXECUTE
        ...
        'WHERE tgt.maintenance_id = src.maintenance_id ';
...
alter procedure job_maintenance_event(text) owner to admin;
